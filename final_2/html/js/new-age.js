	    ...
	    $.ajax({
	      ...
	      success: function(response) {
		        ...
		        if(response.unexpected_error){
					LoadError('Wystąpił nieoczekiwany błąd.');
					HideLoadingPanel();
		        }else if(!response.is_problem_feasible){
					LoadError('Problem jest nieobliczalny.');
					HideLoadingPanel();	
		        }
		        else{
		        	setTimeout(function (){
...
function Validate(){
	var result = true;
	for(var i = 0 ; i < $('#enbCounter').val(); i++){
		if(!ValidateEnbRange(i)){
			result = false;
		}
	}

	for(var i = 0 ; i < $('#routerCounter').val(); i++){
		if(!ValidateRouterRange(i)){
			result = false;
		}
	}
...
function ValidateEnbRange(number){
	...
	if(!result){
		alert("Błąd w ENB numer " + number+ ". Pole wychodzi poza obszar siatki.");
	}
...
function ValidateRouterRange(number){
	...
	if(!result){
		alert("Błąd w routerze numer " + number + ". Pole wychodzi poza obszar siatki.");
	}
    ...
    $(".mastsolve").empty().append(
		'<div class="container h-100">'+
			...
			'<div class="text-center col-md-12">'+
				...
				'<div id="table"></div>'+				
				//'<h4>Rozwiązanie optymalne problemu: ' + response.optimal_solution + '</h4>' +
				'<br><h4>Czas rozwiązania w mikrosekundach: ' + response.time_in_microseconds + '</h4>' +
			'</div>'+
		'</div>'
    	);
