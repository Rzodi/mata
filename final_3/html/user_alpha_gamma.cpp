...
vector<vector<float>> user_alpha_gamma::user_position_demands(int users, float minDemand, float maxDemand, float x_grid, float y_grid, vector<Position> enb_positions, vector<Position> router_positions, float enbR, float routerR)
{
    ...
    for (int i = 0; i < users; i++) {
    	...
    	float x = rand_FloatRange((container[random_enb_or_roouter].X - rangeHelper), (container[random_enb_or_roouter].X + rangeHelper));
    	...
    	float y = rand_FloatRange((container[random_enb_or_roouter].Y - rangeHelper), (container[random_enb_or_roouter].Y + rangeHelper));
